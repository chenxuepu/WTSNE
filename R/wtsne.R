...
#'
#' After checking the correctness of the input, the \code{wtsne} function (optionally) does an initial reduction of the feature space using \code{\link{prcomp}}, before calling the C++ TSNE implementation. Since R's random number generator is used, use \code{\link{set.seed}} before the function call to get reproducible results.
#'
...
#' If a distance matrix is already available, this can be directly supplied to \code{wtsne} by setting \code{is_distance=TRUE}.
#' This improves efficiency by avoiding recalculation of distances, but requires some work to get the same results as running default \code{wtsne} on a data matrix.
...
#'
#' NN search results can be directly supplied to \code{wtsne_neighbors} to avoid repeating the (possibly time-consuming) search.
#' To achieve the same results as \code{wtsne} on the data matrix, the search should be conducted on the normalized data matrix.
...
#' In contrast, running \code{wtsne} on a data matrix will always use Euclidean distances.
#'
...
#' set.seed(42)
#' tsne_out <- wtsne(iris_matrix,pca=FALSE,perplexity=30,theta=0.0) # Run TSNE
#'
#' # Show the objects in the 2D tsne representation
#' plot(tsne_out$Y,col=iris_unique$Species, asp=1)
#'
#' # data.frame as input
#' tsne_out <- wtsne(iris_unique,pca=FALSE, theta=0.0)
#'
#' # Using a dist object
#' set.seed(42)
#' tsne_out <- wtsne(dist(normalize_input(iris_matrix)), theta=0.0)
#' plot(tsne_out$Y,col=iris_unique$Species, asp=1)
#'
#' set.seed(42)
#' tsne_out <- wtsne(as.matrix(dist(normalize_input(iris_matrix))),theta=0.0)
#' plot(tsne_out$Y,col=iris_unique$Species, asp=1)
#'
#' # Supplying starting positions (example: continue from earlier embedding)
#' set.seed(42)
#' tsne_part1 <- wtsne(iris_unique[,1:4], theta=0.0, pca=FALSE, max_iter=350)
#' tsne_part2 <- wtsne(iris_unique[,1:4], theta=0.0, pca=FALSE, max_iter=650, Y_init=tsne_part1$Y)
#' plot(tsne_part2$Y,col=iris_unique$Species, asp=1)
#' \dontrun{
#' # Fast PCA and multicore
#'
#' tsne_out <- wtsne(iris_matrix, theta=0.1, partial_pca = TRUE, initial_dims=3)
#' tsne_out <- wtsne(iris_matrix, theta=0.1, num_threads = 2)
#' }
#' @useDynLib wtsne, .registration = TRUE
#' @import Rcpp
...
#' @describeIn wtsne Default Interface
#' @export
...
#' @describeIn wtsne tsne on given dist object
#' @export
...
#' @describeIn wtsne tsne on data.frame
#' @export
